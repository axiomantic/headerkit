...
- **The Common Defect**: The items below were each diagnosed separately. All are one shape -- a fact the source states, which the IR either cannot express or does not oblige anyone to populate, and whose absence is indistinguishable from "not applicable":
  - `Function.calling_convention` is declared in the IR (`ir.py:707`; `FunctionPointer` carries its own at `:335`) and read by writers across the tree (`ctypes`, `cython`, `nim`, `lua`, `json`, `diff`), yet no backend ever assigns it. The `None` default reads as "default convention", so a `__stdcall` function is silently emitted `cdecl`.
...
- **Zero-Overhead Capability Discovery (No Premature Installs)** -- *landed; the remainder of this item is still planned*:
...
- **Lua Runtime Coverage Beyond LuaJIT (Lua C API Writer)**:
  ...
  - **Proving case**: generate Lua bindings for a Zig library, composing the `tree-sitter-zig` extraction described under *Grammar-Based Polyglot Source AST Extraction* (Next) with the Lua C API writer. This pairing earns its place because neither side is C, making it the first real test of whether the IR is a language-neutral interop contract or a C-shaped pipe with extra writers attached.
