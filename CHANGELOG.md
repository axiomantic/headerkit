...
- Config file discovery: `find_config_file` now checks both `.headerkit.toml` and `headerkit.toml` (without leading dot) before falling back to `pyproject.toml`.
...
- Libclang backend template parameter parsing: `_extract_template_parameters` extracts detailed template parameters and default argument expressions for classes and functions, anchoring parameter searches after the `template` keyword and handling multi-token default values and angle bracket closures accurately.
- `NimWriter` pointer iterator synthesis: automatically emits idiomatic `items*` iterators and `inc*` procs for types with `begin()` and `end()` methods returning pointers.
- Access specifier floor in `NimWriter` and `CythonWriter`: private and protected members (fields, methods, constructors, destructors) and base classes are filtered out by default when emitting bindings. Generating bindings for inaccessible members caused downstream C++ compiler errors when compiling emitted importcpp procs or .pxd declarations against C++ classes with private internals.
- C++17 deduction guide suppression in `LibclangBackend`: synthetic deduction guide declarations (`<deduction guide for ...>`) extracted by libclang are now suppressed during function and method processing. These AST constructs have no mangled entry points, cannot be called at runtime, and produce uncompilable bindings when emitted.
...
- Bumped `IR_SCHEMA_VERSION` from 4 to 5 and cache key format version to incorporate `TemplateParameter` lists and record access levels.
- Target-neutral IR notes for non-type template parameters in foreign language bindings.
...
- `NimWriter` type resolution and overload deduplication:
  ...
  - Anonymous enum constant disambiguation: resolved top-level constant collisions between anonymous enum values and proc/method names by qualifying with enclosing namespace or suffixing with domain-neutral `_val`.
  ...
  - Proc signature canonicalization: canonicalizes alias parameter types in `_emitted_proc_sigs` recursively while preserving distinct generic arguments and array dimensions, preventing silent drops of valid method overloads.
...
- Underscore sanitization in `NimWriter`: `_escape_ident` now replaces consecutive underscores (`__`) and prefixes leading underscores with `uu_` / `uN_` so emitted symbols satisfy Nim's lexer without collision or syntax errors.
- Generic type export markers in `NimWriter`: `_write_struct` now places the export marker `*` immediately after the type identifier (`Tag*[T]`) rather than after generic parameters (`Tag[T]*`), resolving Nim syntax errors.
- C++ assignment operator mapping in `NimWriter`: mapped `"operator="` to `"assign"` instead of illegal backtick identifier `` "`=`" ``, which the Nim lexer rejects with `RenameError`.
- Test harness environment isolation: `_run_pytest` in `tests/test_scaffold_runs.py` and `tests/test_workorder_execution.py` now sanitizes child `PYTEST_ADDOPTS` to an empty string, preventing host pytest options (such as `--last-failed`) from conflicting with isolated sub-pytest runs invoked with `-p no:cacheprovider`.
