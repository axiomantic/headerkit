...
- Updated test subprocess interception dependency distribution name from `python-tripwire` to `pytest-tripwire`. The package was renamed again upstream; the import name (`tripwire`) and plugin proxy attribute (`tripwire.subprocess`) are unchanged. Test minimum is now `pytest-tripwire>=0.20,<1`.
