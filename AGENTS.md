...
`clangir/_clang/` contains vendored upstream clang Python bindings for LLVM 18-21. These are excluded from ruff and mypy. Do not modify, refactor, or lint them.

## Registry pattern

Backends and writers use a managed circular import pattern for self-registration. Both `clangir/backends/__init__.py` and `clangir/writers/__init__.py` lazily import their concrete modules via `_ensure_*_loaded()`, and each concrete module imports `register_backend`/`register_writer` at the bottom of the file to self-register. Do not restructure these imports.
...
`clangir/__init__.py` defines `__all__` with the full public surface. Backends and writers are accessed via registry functions (`get_backend()`, `get_writer()`), not by importing concrete classes directly. When adding new public symbols, add them to both the imports and `__all__` in `__init__.py`.
